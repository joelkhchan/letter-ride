...
- [ ] **Step 1: Find all occurrences** — `grep -rn "addWit" src/ test/` (expected: `scoring.js:24` reads `d.addWit`; `tiles.js` resonator/polished/anchor return `{addWit:...}`; `relics.js` vowelBonus/rareHoarder/doubleTrouble return `{addWit:...}`; plus any test fixtures/assertions).
...
- [ ] **Step 3:** In `src/tiles.js` and `src/relics.js`, change every modifier `evaluate` that returns `{ addWit: X }` → `{ addPoints: X }`. Update any test that constructs a modifier with `addWit` or asserts on it.
...
**Interfaces:** `newRun` adds `honeLevels: {}`; `playsPerRound` also adds `Σ relic.extraPlays`. `playWord` passes to `scoreWord`: `relics: [...run.relics, ...honeModifiers(run.honeLevels)]` and `context: { wordsPlayedThisRound: run.wordsPlayedThisRound, enablers: run.relics.filter(r => r.enabler).map(r => r.enabler) }`.
...
test('relic extraPlays raises playsPerRound at newRun', () => {
  resetTileIds();
  const run = newRun({ config, dictionary: dict, seed: 1, loadout: { startRelics: [{ id:'ep', extraPlays: 1, evaluate:()=>({}) }] } });
  assert.equal(run.playsPerRound, config.PLAYS_PER_ROUND + 1);
});
```
...
- [ ] **Step 3: Implement** — in `src/run.js`:
  ...
  - in `newRun`, add `honeLevels: {}` to the returned state; change `playsPerRound` to `config.PLAYS_PER_ROUND + (stake?.playsDelta||0) + sumExtraPlays(loadout.startRelics)` where `sumExtraPlays = (relics=[]) => relics.reduce((n,r)=>n+(r.extraPlays||0),0)`. (Define the helper at top of run.js. Note: relics bought mid-run via the shop that grant extraPlays apply next round only — acceptable; document.)
...
- [ ] **Step 1: Failing test** — append: serialize→deserialize a run with `run.honeLevels = { rareLetter: 3 }` preserves it.
- [ ] **Step 2: Run, see fail.**
...
- [ ] **Step 1: config** — add `HONE: { cost: 6 }` inside CONFIG.
...
- [ ] **Step 4: Implement** — in `src/shop.js`:
  ...
  - in `purchase`, add a case: `case 'hone': run.honeLevels[offer.archetypeId] = (run.honeLevels[offer.archetypeId] || 0) + 1; break;` (before the `run.coins -= offer.cost` deduction).
...
```
where `hasRareCtx`/`isDoubledCtx` are the SAME enabler-aware predicates as in `archetypes.js` — to avoid duplication, **export `hasRare` and `isDoubled` from `archetypes.js`** and import them into `relics.js` (rename the exported names `hasRareCtx`/`isDoubledCtx` for clarity, or import as-is). Decide one home; do not copy the predicate logic into both files.
...
```
- [ ] Add to `config.DECKS`. Decide unlock state: add to `META.baseUnlocked.decks` OR leave locked behind Meta (per archetype-deck signposting). Default: lock them (unlockable via meta-shop, like vowelHeavy/wildcard).
...
**Files:** Modify `src/telemetry.js`, `scripts/analyze-builds.js`.
- [ ] `telemetry.js`: on each play, classify the word by which `ARCHETYPES[*].matches(ctx)` fire, and accumulate per-archetype play counts + score; `summarize` reports per-archetype avg score + share of plays. (Import `ARCHETYPES`.) Test the classification accumulation.
- [ ] `analyze-builds.js`: extend to score a representative *committed build* per archetype (the archetype's best word + its relic(s) + a few hone levels) vs the no-relic long baseline, printing each archetype's ratio — so the author can see all ~6 head-to-head, enforcing "each can win." (No assertions; instrument.) Run `npm run analyze`, capture output.
- [ ] Commit `feat: per-archetype telemetry + head-to-head harness`.

> **🛑 ARCHETYPE GATE (author playtest).** Play runs committing to each archetype. Use the Stats view + `npm run analyze` to check every archetype can win (StS bar). Tune (in `config.js` / `archetypes.js` / `relics.js`): hone increments, ×Mult magnitudes, enabler power vs. the scarcity pillar. This is the author's balance call.
...
- **Engine untouched:** `scoreWord` gets no new params — hone rides the existing `relics` array; enablers ride the existing `context`. Confirm the current additive-points delta key name in scoring.js (`addPoints`) before writing modifiers.
