...
**Date:** 2026-06-22 (rev. 3, 2026-06-23: reconciled with the full research dump; rev. 2 post spec-review + author discussion) · **Author call:** Joel. **Status:** active strategic roadmap; supersedes the "deferred wishlist" of the original 4-tier plan (`docs/2026-06-20-letter-ride-plan.md`). Grounded in the design spec, the genre-research docs (`2026-06-23-letter-ride-competitive-research.md`, `-empirical-findings.md`, `-research-appendix.md`), and the archetype framework. The phase *sequence* predates the 2026-06-23 deep research but is reinforced by it; the research's deep findings are integrated into Phase 1 below and detailed in those docs.
...
- **0c — Tune to the gate:** author plays (the *fun* judgment) + eval win-rate data (the *balance* signal). Tune hone increments, enabler power, the target ladder. **Scope (post-2026-06-23 research): a *light* pass.** Lower `ROUND_TARGETS` just enough that a skilled line clears the median, so the basic loop can be fun-validated. Do **not** perfect the curve here: Phase 1 redesigns the curve around the scaling engine, so over-tuning 0c is throwaway. 0c validates whether the basic loop is pleasant and winnable, not the full exponential hook (Phase 1+). Don't over-balance — preserve the "break it" highs.
...
Map the scoring system's real limits and **lock the mechanical vocabulary**, so bosses/events/content are designed coherently. Powered by the eval harness (it *measures* the boundaries). Outputs a short **"systems bible"** doc.

**The structural finding driving this phase (2026-06-23 harness + research):** the scoring *math* already supports exponential (`mult = (1 + ΣaddMult) × ΠtimesMult`; the product is multiplicative), but the *content* does not. Every ×Mult relic is a fixed constant and **nothing compounds across rounds** (combo/momentum reset each round), so linear relic acquisition cannot beat an exponential target. That is why even skilled personas win ~0% (`empirical-findings.md`). The spine's core job is therefore the **additive → multiplicative build *arc*** (+Points early, +Mult mid, ×Mult plus scaling late) and the **scaling engines** that let a committed build ride a steep curve.

Covers:
- **Score-growth curve & ceiling:** with the best legal build, max score per round vs. the target ladder `[40…600]` — headroom or trivialized? Benchmark shape: Balatro runs **~2.2×/round, front-loaded, with a Small/Big/Boss rhythm** (300 → 50k over 8 antes); decide our curve and whether targets *chase* the engine's ceiling.
- **The ×Mult blowup:** how many `×Mult` sources stack and the resulting product. Given "breaking it is fun," the question is *how high we let it go* and *whether targets chase it* — a deliberate dial, not a cap. **Precision guard:** at blowup scale, protect against Balatro's "naneinf" failure by keeping scores representable at the arithmetic *and* display layers, so `score >= target` never breaks.
- **Effect vocabulary** *(candidate-IN: all)*: beyond `+Points/+Mult/×Mult` — retriggers, tile transform/destroy, per-word-property effects, and the **negative/warp effects bosses need** (disable / cap / tax / lock). Organize relics by the **role taxonomy** (`competitive-research.md`): flat +Points · flat +Mult · conditional ×Mult · **scaling/snowball** · **×Mult wincon** · economy · **retrigger** · **copy/engine**. We ship only the first three plus economy today; the bolded roles are the compounding gap.
- **Hone as the scaling-investment axis** *(now built, re-evaluate)*: Hone is our Planet-card analog (it permanently levels an archetype within a run). Decide its **granularity** (level a relic, a scoring axis, or a word-structure) and whether high levels should emit **×Mult** (today it only adds +Points/+Mult and resets each run). This is the most likely home for the cross-round compounding the harness says we lack.
