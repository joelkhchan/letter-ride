...
test('bossOrder is seeded + deterministic + one of each boss', () => {
  ...
  assert.deepEqual([...a.bossOrder].sort(), ['censor','margin','mute','oneLiner','toll','vise']);
  assert.equal(a.boss, null);                 // encounter 0 is a Word, no boss
});

test('The Vise allows no discards on its boss encounter', () => {
  ...
  assert.equal(run.discardsLeft, 0);                       // keep:0 applied at encounter setup (no discards this round)
});

test('The One-Liner caps plays at 1 and lowers the target on its boss encounter', () => {
  const run = newRunB({ config: bossCfg, dictionary: dictBoss, seed: 5 });
  run.bossOrder = ['oneLiner','mute','toll','margin'];
  const baseTarget = run.targets[2];
  nextRoundB(run); nextRoundB(run);                        // Passage 1 Sentence, boss = oneLiner
  assert.equal(run.boss, 'oneLiner');
  assert.equal(run.playsLeft, 1);                          // capped to 1 play
  assert.ok(run.target < baseTarget, 'target lowered below the un-warped round target');
});
