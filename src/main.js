...
import { EVENTS, applyEventOption, pressStart, pressDraw, pressBank, proofStart, proofGuess, proofClaim } from './events.js';
...
try {
  ...
  // Common-word answer pool for The Proof (word-deduction event). Resilient: an empty pool just disables the event's target.
  const proofAnswers = await fetch('assets/proof-answers.txt').then(r => r.ok ? r.text() : '').then(t => t.split(/\r?\n/).filter(Boolean)).catch(() => []);
  ...
  bindControls({
    ...
    onPickEvent() {
      ...
      if (ev.interactive) {
        if (ev.id === 'theProof') proofStart(run, proofAnswers);
        else pressStart(run);
    ...
    // The Proof (word-deduction event): submit a guess; the board re-renders with feedback.
    onProofGuess(word) {
      const r = proofGuess(run, word);
